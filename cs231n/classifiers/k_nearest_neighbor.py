...
class KNearestNeighbor(object):
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    print "no loops test"
    distances = (X ** 2).sum(axis=1, keepdims=True) \
            + (self.X_train ** 2).sum(axis=1) \
            - 2 * X.dot(self.X_train.T)
    
    dists = np.sqrt(distances)
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      #########################################################################
      distances_to_training = dists[i]
      sorted_indices = np.argsort(distances_to_training)[:k]
      ...
      # label.                                                                #
      #########################################################################
      max_count = np.bincount(closest_y)
      max_label = np.argmax(max_count)
      y_pred[i] = max_label
      #########################################################################
      #                           END OF YOUR CODE                            # 
      #########################################################################
